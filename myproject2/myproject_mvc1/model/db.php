...
class DBConnection {
    ...
    public static function connect() {
    	try {
    		...
    		self::$_db = new PDO("mysql:host=$sHost;dbname=$sDB", self::$_user, self::$_password );
    		//print_r($db);die;
    ...
    public function Select() {
    	...
    	if (!empty($table)) {
    		...
    		$this->_result = self::$_db->prepare($this->_query);
    		$bool = true;
    	}
    	return $bool;
    }
    ...
    public function Insert() {
    	...
    	if (($countKey > 0 && $countValue > 0) && $countKey == $countValue) {
    		...
    		if (!empty($table)) {
    			...
    			$sql = self::$_db->exec($this->_query);
    
    			$this->_insertId = self::$_db->lastInsertId();
    			if ($this->_insertId > 0) {
    				$bool = true;
    			}
    		}
...
//     }
    ...
    public function resultArray() {
    	...
    	if (! empty ( $this->_query )) {
    		echo '<br/>';
    		echo "<pre>";
    		foreach ( self::$_db-> query ( $this->_query ) as $row ) {
    			echo "<br/>";
    			print_r ( $row );
